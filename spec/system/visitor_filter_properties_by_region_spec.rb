...
describe 'Visitor filter properties by region' do
    it 'should view links in nav bar' do
        #Arrange
        PropertyRegion.create!(name: 'Bertioga')
        PropertyRegion.create!(name: 'Guarujá')
    ...
    it 'successfully' do
        ...
        click_on 'Bertioga'
        #Assert
        expect(page).not_to have_content('Apê na baixada Santista')
        expect(page).to have_link('Casa confortável no litoral')
    end
end